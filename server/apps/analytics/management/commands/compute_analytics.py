    ...
    def handle(self, *args, **options):
        ...
        if exam_code:
            ...
            try:
                ...
                raise CommandError(f'Exam with code "{exam_code}" does not exist')
            ...
            self.stdout.write(self.style.SUCCESS(
                f'✓ Component aggregates computed: {len(result["component_aggregates"])} components'
            ))
            self.stdout.write(self.style.SUCCESS(
            ...
            for exam in exams:
                try:
                    result = compute_all_analytics(exam)
                    success_count += 1
                    self.stdout.write(f'  ✓ {exam.code}: {result["exam_aggregate"].total_students} students')
